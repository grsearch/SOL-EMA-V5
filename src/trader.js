...
// 是否优先使用 Pump AMM 直接卖出（绕过 Jupiter，减少 500-900ms 延迟）
// 设为 true 启用，false 回退到 Jupiter
const USE_PUMP_AMM_SELL = process.env.USE_PUMP_AMM_SELL !== 'false';
...
async function sell(tokenState, fraction, reason) {
  ...
  try {
    // ── 优先路径：直接调 Pump AMM（快 500-900ms）────────────────
    const poolAddress = tokenState.position?.poolAddress;
    if (USE_PUMP_AMM_SELL && poolAddress) {
      try {
        logger.info(`[Trader] 尝试直接 AMM 卖出 pool=${poolAddress.slice(0, 8)}`);
        const ammResult = await pumpAmm.sellViaAMM({
          conn:        getConn(),
          keypair:     getKeypair(),
          poolAddress,
          tokenMint:   address,
          tokenAmount: rawSellAmount,
          isRug,
        });

        const solReceived = ammResult.solReceived;
        const newBalance  = position.tokenBalance - rawSellAmount;

        logger.warn(
          `[Trader] AMM SELL OK ${symbol}` +
          ` | sig=${ammResult.signature?.slice(0, 12)}` +
          ` | received≈${solReceived.toFixed(4)} SOL` +
          ` | elapsed=${ammResult.elapsed}ms`
        );
        _broadcastTrade('SELL', symbol, address, currentPrice, solReceived, ammResult.signature, reason);

        if (newBalance <= 0) return { solReceived };
        return { ...position, tokenBalance: newBalance, solReceived };

      } catch (ammErr) {
        logger.warn(`[Trader] AMM卖出失败，回退Jupiter: ${ammErr.message}`);
        // 继续走 Jupiter 路径
      }
    }

    // ── 回退路径：Jupiter Ultra API ──────────────────────────────
    const result = await executeWithRetry(
    ...
    if (newBalance <= 0) return { solReceived };   // 全仓卖出，返回实际收入
    return { ...position, tokenBalance: newBalance, solReceived };
  } catch (e) {
...
async function exitPosition(tokenState, reason) {
  if (!tokenState.position || tokenState.position.tokenBalance <= 0) return null;
  const result = await sell(tokenState, 1.0, reason);
  tokenState.position = (result && result.tokenBalance > 0) ? result : null;
  return result;  // 返回包含 solReceived 的结果
}
